...
function detectBrowserLanguage() {
    ...
    const navigatorWithUserLanguage = navigator;
...
function updateUITexts() {
    if (translations.ui) {
        ...
        if (pageSubtitle)
            pageSubtitle.textContent = translations.ui.subtitle;
        if (partsLabel)
            partsLabel.textContent = translations.ui.partsCollected;
        if (instrumentsLabel)
            instrumentsLabel.textContent = translations.ui.completeInstruments;
        if (songsLabel)
            songsLabel.textContent = translations.ui.completeSongs;
        if (resetBtn)
            resetBtn.textContent = translations.ui.resetProgress;
        if (exportBtn)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (songsCount)
        songsCount.addEventListener('click', scrollToCompleteSongs);
